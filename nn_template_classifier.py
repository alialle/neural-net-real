"""
Reusable Neural Network Training Template (Classification)
--------------------------------------------------------
Uses scikit-learn's MLPClassifier to predict a CATEGORY (e.g. yes/no,
type A/B/C) from your own CSV data (e.g. exported from SQL). Rename
the placeholders below to match your actual column names and you're
ready to go.
"""
...
INPUT_COLUMNS = ["feature_1", "feature_2", "feature_3"]  # your input columns
OUTPUT_COLUMN = "target_category"   # the category you're trying to predict
...
y = df[OUTPUT_COLUMN].astype(str).to_numpy()  # the category it's trying to predict
...
# For classification, "loss" here is log loss (penalizes confident wrong
# predictions more than mean squared error would).
...
for epoch in range(1, MAX_ITER + 1):
    ...
    test_loss_this_epoch = log_loss(y_test, model.predict_proba(X_test), labels=model.classes_)
...
print(f"Best test loss (log loss): {best_test_loss:.4f}  (found at epoch {best_epoch} of {MAX_ITER})")
...
# ============================================================
# "Loss" here = log loss (lower is better). We also print accuracy
# (% of predictions that were exactly correct) since that's often more
# intuitive for classification than log loss alone.
...
train_loss = log_loss(y_train, model.predict_proba(X_train), labels=model.classes_)
test_loss = log_loss(y_test, model.predict_proba(X_test), labels=model.classes_)
test_accuracy = accuracy_score(y_test, model.predict(X_test))
print(f"Training loss: {train_loss:.4f}   |   Test loss: {test_loss:.4f}   |   Gap: {abs(train_loss - test_loss):.4f}")
